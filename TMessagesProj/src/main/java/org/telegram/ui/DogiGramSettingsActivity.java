...
public class DogiGramSettingsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    // DogiGram: Screenshot Mode replaces every name with a random-looking 8-digit code that is
    // NOT the real user/chat id. The salt is generated once per install so the codes stay stable
    // within an install (no flicker on redraw) but cannot be reversed back to the real id.
    private static long screenshotSalt = 0L;

    private static long screenshotSalt() {
        if (screenshotSalt == 0L) {
            SharedPreferences p = prefs();
            long s = p.getLong("screenshot_salt", 0L);
            if (s == 0L) {
                s = new java.security.SecureRandom().nextLong() | 1L; // keep it non-zero
                p.edit().putLong("screenshot_salt", s).apply();
            }
            screenshotSalt = s;
        }
        return screenshotSalt;
    }

    // DogiGram: deterministic 8-digit code (10000000..99999999) derived from an id.
    public static int screenshotCode(long id) {
        long h = id ^ screenshotSalt();
        h ^= (h >>> 33);
        h *= 0xff51afd7ed558ccdL;
        h ^= (h >>> 33);
        h *= 0xc4ceb9fe1a85ec53L;
        h ^= (h >>> 33);
        return (int) (Math.floorMod(h, 90000000L) + 10000000L);
    }

    // DogiGram: anonymised labels shown across the UI while Screenshot Mode is on.
    public static String screenshotUserName(long userId, boolean bot) {
        return (bot ? "Bot " : "User ") + screenshotCode(userId);
    }

    public static String screenshotChatName(long chatId) {
        return "Chat " + screenshotCode(chatId);
    }
    ...
    public View createView(Context context) {
        ...
        listView.setOnItemClickListener((view, position) -> {
            ...
            if (position == drawerUiRow) {
                ...
                presentFragment(new MessageMenuSettingsActivity());
    ...
    private void applyScreenshotMode(boolean enabled) {
        // DogiGram: Screenshot Mode no longer blocks screenshots (FLAG_SECURE). It only anonymises
        // names and hides the phone number, so make sure the secure flag is always cleared here
        // (also undoes the flag for anyone upgrading from a build that still set it).
        if (getParentActivity() != null) {
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public boolean isEnabled(RecyclerView.ViewHolder holder) {
            ...
            return position == drawerUiRow || position == showPhoneRow || position == screenshotRow
                    ...
                    || position == doubleTapRow || position == messageMenuRow || position == serverStatusRow
            ...
            switch (holder.getItemViewType()) {
                ...
                case 3: {
                    ...
                    if (position == doubleTapRow) {
                        ...
                        settingsCell.setText("Message Menu", true);
                    } else if (position == serverStatusRow) {
        ...
        public int getItemViewType(int position) {
            if (position == headerRow || position == featuresHeaderRow || position == extraHeaderRow || position == aboutHeaderRow) {
                ...
                return 2;
            } else if (position == doubleTapRow || position == messageMenuRow || position == serverStatusRow || position == maxRecentStickersRow

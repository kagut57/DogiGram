...
public class DogiConfig {
    ...
    // --- Message menu (chat message context menu) visibility -----------------------------------
    // DogiGram: let the user pick which entries appear in the long-press message menu. Each option
    // is keyed by its ChatActivity OPTION_* id and defaults to visible; only the managed ids below
    // can be toggled, everything else is always shown.

    private static final String MENU_OPTION_PREFIX = "msg_menu_opt_";

    public static boolean isMessageMenuOptionEnabled(int option) {
        return prefs().getBoolean(MENU_OPTION_PREFIX + option, true);
    }

    public static void setMessageMenuOptionEnabled(int option, boolean enabled) {
        prefs().edit().putBoolean(MENU_OPTION_PREFIX + option, enabled).apply();
    }

    public static void toggleMessageMenuOption(int option) {
        setMessageMenuOptionEnabled(option, !isMessageMenuOptionEnabled(option));
    }
}
